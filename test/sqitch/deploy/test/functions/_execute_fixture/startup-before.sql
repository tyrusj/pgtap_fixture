-- Deploy dream-db-extension-tests:functions/_execute_fixture/startup-before to pg

BEGIN;

create function unit_test.test_func_execute_fixture__startup_before()
returns setof text
language plpgsql
as
$$
declare test_description text := $test_description$
When executing a fixture, if the fixture has a non-null startup statement, then the function shall
execute the startup statement before executing the first child test or child fixture in the plan.
$test_description$;
...
begin
    ...
    -- Create the fixtures
    insert into fixture ("name", "startup") values ('my_fixture_a', $startup$insert into execution ("order", "executed") values (nextval('execution_order'), 'fixture startup')$startup$) returning id into fixture_id;
    ...
    -- Add the child fixture and child test to the plan
    ...
    return query select tap.ok(
        ...
        $scenario$1: Child fixture executed after the fixture startup.$scenario$)
    );

    return query select tap.ok(
        fixture_order < test_order,
        format(E'%s\ntest_scenario: %s', test_description,
        $scenario$2: Child test executed after the fixture startup.$scenario$)
    );
