-- Deploy dream-db-extension-tests:functions/_execute_test/rollback-on-teardown-exception to pg

BEGIN;

create function unit_test.test_func_execute_test__rollback_on_teardown_exception()
returns setof text
language plpgsql
as
$$
...
test, if the test's parent fixture's teardown statement throws an exception, then the database shall
...
begin
    ...
    values (
        'my_fixture',
        $setup$insert into modified_table ("value") values ('fixture setup modification');$setup$,
        $teardown$
        insert into modified_table ("value") values ('fixture teardown modification');
        raise 'my_fixture_teardown exception';
        $teardown$
    ...
    return query select tap.set_hasnt(
        'select value from modified_table;',
        $hasnt$select 'fixture teardown modification'$hasnt$,
        format(E'%s\ntest_scenario: %s', test_description,
        $scenario$3: Expect changes made by the fixture teardown to be rolled back.$scenario$)
    );
