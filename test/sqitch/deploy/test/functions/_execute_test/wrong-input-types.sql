-- Deploy dream-db-extension-tests:test/functions/_execute_test/wrong-input-types to pg

BEGIN;

create function unit_test.test_func_execute_test__wrong_input_types()
returns setof text
language plpgsql
as
$$
declare test_description text := $test_description$
When executing a test, if the test function does not take one of the following sets of inputs, then
the function shall fail the test: (), (jsonb, text)
$test_description$;
...
begin
    ...
    -- Create test function with the wrong input types
    create function pg_temp.my_test_function(text, text)
    returns text
    language plpgsql
    as
    $test$
