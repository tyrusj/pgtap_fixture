-- Deploy dream-db-extension-tests:functions/_execute_test/return-results to pg

BEGIN;

create function unit_test.test_func_execute_test__return_results()
returns setof text
language plpgsql
as
$$
declare test_description text := $test_description$
When executing a test with data, if the test function takes parameters, and if test data are associated
with the given test, then for each record in the test data table that corresponds with the given test,
the function shall return the twice indented test results of the test executed with that data.
$test_description$;
...
begin
    ...
    -- Create test data
    insert into test_data ("test_id", "parameters") values (test_id, $params$select '["return value A"]'$params$), (test_id, $params$select '["return value B"]'$params$);
    ...
    return query select tap.set_has(
        ...
        $has$select v.value from (values ('        return value A'), ('        return value B')) v(value)$has$,
        test_description
    );
