-- Deploy dream-db-extension-tests:functions/_execute_test/teardown-after-test to pg

BEGIN;

create function unit_test.test_func_execute_test__teardown_after_test()
returns setof text
language plpgsql
as
$$
...
given test, if the test has a parent fixture, and if that fixture has a teardown statement, then the
function shall execute that teardown statement after executing the test.
$test_description$;
...
begin
    ...
    -- Create a sequence that will track what order the teardown and test function are executed in.
    ...
    -- Create a fixture
    insert into fixture ("name", "teardown") values ('my_fixture', $setup$insert into execution ("order", "executed") values (nextval('execution_order'), 'fixture teardown');$setup$) returning id into fixture_id;
    ...
    return query select tap.results_eq(
        ...
        $expect$select v.executed from (values ('fixture teardown')) v(executed)$expect$,
        format(E'%s\ntest_scenario: %s', test_description,
        $scenario$1: Execute fixture teardown after test with data params1.$scenario$)
    );
    ...
    return query select tap.results_eq(
        ...
        $expect$select v.executed from (values ('fixture teardown')) v(executed)$expect$,
        format(E'%s\ntest_scenario: %s', test_description,
        $scenario$2: Execute fixture teardown after test with data params2.$scenario$)
    );
