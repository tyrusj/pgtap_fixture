...
create function unit_test.test_func_execute_test__return_result()
returns setof text
language plpgsql
as
$$
declare test_description text := $test_description$
When executing a test with data, if the test function does not take arguments, or if the test function
takes arguments and there are test data associated with the test, then the function shall return a
formatted result, where the result name is the schema and function of the test, and the result
...
begin
    ...
    return query select tap.set_has(
        ...
        $want$values ('ok 1 - pg_temp.my_test_function_no_args my description no args')$want$,
    ...
    return query select tap.set_has(
        ...
        $want$values ('ok 1 - pg_temp.my_test_function_with_args my description args')$want$,
        format(E'%s\ntest_scenario: %s', test_description,
